...
-- Version corrigée sans contrainte CHECK avec sous-requête
...
-- 7. Créer une fonction de validation pour vérifier l'existence des formations
CREATE OR REPLACE FUNCTION public.validate_formation_item()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.item_type = 'formation' THEN
        IF NOT EXISTS (SELECT 1 FROM public.formations WHERE id = NEW.item_id) THEN
            RAISE EXCEPTION 'Formation avec ID % non trouvée', NEW.item_id;
        END IF;
    END IF;
    RETURN NEW;
END;
...
-- 8. Créer un trigger pour valider les formations avant insertion
DROP TRIGGER IF EXISTS validate_formation_trigger ON public.order_items;
CREATE TRIGGER validate_formation_trigger
    BEFORE INSERT OR UPDATE ON public.order_items
    FOR EACH ROW
    EXECUTE FUNCTION public.validate_formation_item();

-- 9. Créer une vue pour les statistiques des formations
...
-- 10. Script de test pour vérifier l'intégration
